...
def main():
    ...
    # 5. Express the factors in the decoded-log-rate variance-ordered basis (factor_order;
    #    report eq:rot). W = C^T V_L is a decoder-induced linear projection (NOT an orthogonal
    #    rotation); with eig-normalized C the latent is ~whitened, so it is close to a reorder.
    C = model.decoder.decode.weight.detach().cpu().numpy()      # (N, L) decoder loading
    W, fve, ctr = decoded_variance_basis(xbar, C)               # W (L,L), fve (L,), center (L,)
    pf, pc, pbar = project(xfilt, W, ctr), project(xfc, W, ctr), project(xbar, W, ctr)
    np.savez(os.path.join(VID, "forecast_paths.npz"), xfilt=xfilt, xfc=xfc, xbar=xbar,
             W=W, fve=fve, C=C, t0=T0, trial_i=best_i)          # raw latents + basis, for replotting
    render(pf[:, :3], pc[:, :3], pbar[:, :3], T0, cfg, res, data, best_i, timing)  # 3D: top-3 factors
    render_slices(pf, pc, pbar, fve, T0, cfg, res, data, best_i)
...
def render(pf, pc, pbar, t0, cfg, res, data, trial_i, timing):
    ...
    def draw(f):
        ...
        ax.set_xlabel("factor 1", labelpad=-8); ax.set_ylabel("factor 2", labelpad=-8); ax.set_zlabel("factor 3", labelpad=-8)
...
def render_slices(pf, pc, pbar, fve, t0, cfg, res, data, trial_i):
    ...
    the forecast tracks the cycle only briefly, then relaxes toward the mean."""
    ...
    fig, axes = plt.subplots(n, 1, figsize=(FW(1.0), 1.0 * n + 0.5), sharex=True)  # scale-1.0 at \textwidth
