...
are not unique and a PCA of the latent itself (which ignores $C$) is not the natural display
basis. We order the factors instead by the variance they explain in the \emph{decoded
log-rate}. Let $\tilde X$ be the centered latent path over a trial (rows $(x_t-\bar x)^\trp$,
...
free-run forecast (\cref{fig:freerun}); the ordering is decoder-aware, not a PCA of the latent
itself.
...
\begin{figure}[t]
  ...
  \caption{Held-out free-run forecast of the regularized model ($L{=}4$, \num{1600} RBF,
    ...
    end of the first cycle (dashed line), with \emph{no} observations after $t_0$. The smoothed
