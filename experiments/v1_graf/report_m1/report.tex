...
against the held-out future spikes by a Poisson-deviance \emph{forecasted-reconstruction
skill}, versus two baselines -- persistence and the stimulus-locked PSTH -- over half, one,
...
roughness) while leaving the (small) forecast skill and the PSTH gap unchanged. The penalty
...
\begin{figure}[t]
  ...
  \caption{Orientation tuning for the six best-fit well-tuned neurons (gray: mean
    ...
    \cref{sec:criterion}) or the forecast skill reported later.}
  \label{fig:tuning}
\end{figure}
...
report the \emph{forecasted-reconstruction skill} $\skl_k=1-D_{\mathrm{model}}(k)/D_{\mathrm{base}}(k)$
...
selection score is $S=0.5\,\skl_8+0.3\,\skl_{16}+0.2\,\skl_{32}$ versus persistence, gated by a
...
horizon} ($\skl_k$ vs.\ PSTH $\approx{-}0.03$ to ${-}0.04$; \cref{fig:reg}B). Within this search, capacity
is not the limiting factor: the best validation skill is flat across $L{=}3$--$5$ and
...
\begin{table}[t]
  \centering
  \caption{Capacity does not lift the forecast. Best validation forecasted-reconstruction skill
...
\emph{not} change the forecast skill, however: on the test trials the smoothed flow
...
\begin{figure}[t]
  ...
  \caption{\textbf{A:} the curvature penalty smooths the one-step map (dir \ang{225}, $L{=}4$,
    ...
    free-run trajectory roughness (blue) collapse, while the forecast skill vs persistence
    ...
    forecasted-reconstruction skill \emph{on the test trials} by horizon (half/one/two cycles):
