...
that ring. Second, forecast accuracy is opposed to sustained oscillation: the only consistent
persistence-beater is the most \emph{contractive} flow ($S\!\approx\!{+}0.012$), which regresses
toward the mean orbit, while flows that sustain the oscillation score worse, and a multi-step
rollout objective trades one axis for the other. Whether the free-run beats the stimulus-locked
PSTH is \emph{direction-dependent}: on the strongest, cleanest direction the PSTH is a near-perfect
phase-locked oracle and is not beaten, but on three noisier directions the free-run beats it at
most horizons -- the learned dynamics add forecast value over trial-averaging where single trials
carry structure the average washes out. The oscillation amplitude itself is only weakly identified
from $\sim$8-cycle trials. The loop runs in real time (\SI{0.3}{ms}/bin).
\end{abstract}

\section{Introduction}
\svjf{} learns latent dynamics and a linear-Gaussian/Poisson readout jointly from a streaming
sequence of observations \cite{Zhao2017c}. The additions used here are (i) a warm-started
...
We use the Graf et al.\ \cite{Graf2011-si} anesthetized-macaque Utah-array recordings:
...
array analyzed by vLGP \cite{Zhao2016a}, binned at \SI{10}{ms}; we analyze the first \SI{1400}{ms} of
...
(bits/spike; \cite{Zhao2016a}): for each held-out neuron, filter the trial from the other neurons'
...
\begin{figure}[t]
  ...
  \caption{Held-out free-run of the learned flow on one test trial, the two leading
    decoded-variance factors (rows; \cref{eq:rot}), for two directions. \textbf{Gold:} the
    trial-averaged latent -- the \emph{PSTH-equivalent} predictor (decoding it gives $\approx$ the
    PSTH rate). \textbf{Teal:} the single-trial filtered latent. \textbf{Crimson:} the autonomous
    free-run launched at the end of the second cycle (dashed; no observations after $t_0$).
    \textbf{Left, \ang{225}} (strongest, cleanest): the gold trial-average tracks the single trial
    markedly better than the free-run, which loses amplitude over the trial -- this is the one
    direction where the PSTH is unbeaten (\cref{fig:generality}); whether the free-run settles onto
    a ring or a point is near-marginal (\cref{fig:limitcycle}). \textbf{Right, \ang{105}}
    (noisier): the free-run tracks the single trial about as well as the trial-average -- here it
    \emph{beats} the near-oracle PSTH at every horizon (\cref{fig:generality}).}
  \label{fig:freerun}
\end{figure}
...
($S{=}{+}0.012\!\pm\!0.003$), whose flow hugs the trial-average orbit and shrinks single-trial
...
\begin{figure}[t]
  ...
  \caption{Held-out forecast accuracy vs horizon (dir \ang{225}, mean$\pm$s.d.\ over \num{8} fit
    seeds), from half a cycle to six. \textbf{Left, vs persistence:} the most contractive flow (single-trial rollout) leads
...
\emph{Forecasting:} only the short-horizon single-trial target consistently beats persistence;
the trial-average target is marginally positive, and the periodic targets -- which sustain the
cycle most strongly -- score worse as the horizon grows. \emph{Dynamics:} the rollout reshapes the
attractor, but its amplitude and stability are weakly identified from $\sim$8-cycle trials. The
long-horizon periodic target, which most directly penalizes amplitude decay, tends to enlarge the
ring toward the data orbit, but unreliably -- ring/data spans $\approx\!0.2$--$1.2$ across the
\num{8} seeds and the center spectral radius $\rho$ straddles $1$ -- and at a forecast cost.
...
\paragraph{Generality across grating directions.}
To test whether these findings are specific to the strongest direction, we repeat the multi-seed
study (three seeds, with the divergence-prone long-horizon rollouts guarded) on three further
directions (\ang{45}, \ang{105}, \ang{295}; PSTH-ceiling PLL \num{0.62}/\num{0.66}/\num{0.58}, all
weaker than \ang{225}'s \num{0.73}). Two of the three findings generalize cleanly: the single-trial
(contractive) rollout is again the best or tied-best forecaster, and the dynamics remain
near-marginal ($\rho$ straddles $1$ on every direction). The PSTH result, however, is
\emph{direction-dependent} (\cref{fig:generality}). On \ang{225} no free-run beats the near-oracle
PSTH at any horizon; but on the three noisier directions the single-trial free-run \emph{beats} it
-- on \ang{105} at every horizon ($a_8\!\approx\!{+}0.06$, $a_{96}\!\approx\!{+}0.04$), on \ang{295}
and \ang{45} at the longer horizons. Where the response is strong and stereotyped (\ang{225}) the
stimulus-locked average is a near-perfect phase-locked predictor an autonomous free-run cannot
beat; where single trials carry structure the average washes out (the noisier directions), the
free-run -- launched from the trial-specific $t_0$ state -- adds genuine forecast value over
trial-averaging.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{generality.png}
  \caption{Generality across four grating directions: held-out forecast accuracy of the
    single-trial rollout vs horizon (mean$\pm$s.d.\ over seeds), against persistence (left) and the
    near-oracle PSTH (right). \textbf{Right:} only the strongest, cleanest direction (\ang{225},
    PLL \num{0.73}) stays below the PSTH at all horizons; the three noisier directions
    (\ang{105}/\ang{295}/\ang{45}) beat it -- most on \ang{105}, at every horizon. The autonomous
    dynamics add forecast value over trial-averaging where the PSTH is a weaker oracle.}
...
\begin{table}[t]
  ...
  \caption{Multi-step rollout study (dir \ang{225}, $L{=}4$, \num{1600} RBF; held-out test,
    \num{8} fit seeds -- 3 local + 5 on GCP). Forecasting and sustained oscillation are opposed:
    the only consistent persistence-beater ($S{>}0$, \textbf{bold}) is the most contractive flow
    (single-trial target), while the flows that enlarge/sustain the oscillation (one-step,
    periodic, long horizon) score worse; none beats the phase-locked PSTH ($a_k$ vs PSTH
    ...
    near-marginal -- across the \num{8} seeds the one-step $\rho$ spans $0.65$--$2.02$ (three
    below $1$, five above), so the focus/limit-cycle label flips fit-to-fit. $S$ and ring/data are
    mean$\pm$s.d.; $a_8$, $a_{96}$, and $\rho$ are seed means. $k_{\max}$ in bins (16/cycle).}
  \label{tab:rollout}
  \begin{tabular}{lccrrrcc}
    \toprule
    rollout target & $k_{\max}$ & clip & $S$ vs persist & $a_8$ PSTH & $a_{96}$ PSTH & ring/data & $\rho$ \\
    \midrule
    none (one-step)  & --  & --  & $-0.008{\scriptstyle\pm0.014}$          & $-0.023$ & $-0.032$ & $0.55{\scriptstyle\pm0.26}$ & $1.30$ \\
    single-trial     & 48  & 1   & $\mathbf{+0.012{\scriptstyle\pm0.003}}$ & $-0.003$ & $-0.019$ & $0.39{\scriptstyle\pm0.17}$ & $0.89$ \\
    trial-average    & 48  & 1   & $+0.006{\scriptstyle\pm0.006}$          & $-0.010$ & $-0.021$ & $0.56{\scriptstyle\pm0.17}$ & $1.42$ \\
    periodic         & 48  & 1   & $-0.001{\scriptstyle\pm0.009}$          & $-0.017$ & $-0.027$ & $0.53{\scriptstyle\pm0.13}$ & $1.22$ \\
    periodic         & 96  & 10  & $-0.004{\scriptstyle\pm0.011}$          & $-0.019$ & $-0.030$ & $0.57{\scriptstyle\pm0.27}$ & $1.28$ \\
    periodic         & 160 & 10  & $-0.007{\scriptstyle\pm0.010}$          & $-0.022$ & $-0.032$ & $0.60{\scriptstyle\pm0.27}$ & $1.44$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
across fits -- and the amplitude is weakly identified from $\sim$8-cycle trials. On the other axis,
\emph{single-trial forecast accuracy does not reward sustaining the oscillation}: against
persistence the best forecaster is the most contractive flow, because under single-trial noise and
phase drift the minimum-deviance prediction regresses toward the mean orbit, and the multi-step
rollout makes the trade-off explicit (short-horizon single-trial targets buy accuracy by
contracting; long-horizon periodic targets sustain the oscillation but forecast worse). Whether the
free-run beats the near-oracle PSTH is direction-dependent and, encouragingly, mostly favorable: on
the strongest, cleanest direction (\ang{225}) the stimulus-locked average is a near-perfect
phase-locked predictor an autonomous free-run cannot beat, but on three noisier directions the
free-run beats it at most horizons -- the learned dynamics add forecast value over trial-averaging
precisely where single trials depart from the average. So the autonomous model is not merely
re-deriving the PSTH: it captures trial-specific dynamics that trial-averaging discards.
