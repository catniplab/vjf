...
ARMS = {
    "srrls_base":     dict(flow="srrls", grow=False, dyn_noise=0.0),
    "sgd_noise":      dict(flow="sgd", grow=False, dyn_noise=0.2, optimizer="adam", lr=1e-4),
    "sgd_grow_noise": dict(flow="sgd", grow=True, grow_weight_init="residual", dyn_noise=0.2,
                           optimizer="adam", lr=1e-4, max_rbf=400),
}
# L=3 across all direction counts; L=6 only for the smaller counts (L=6 x 72 dirs is slow).
DIM_DIRS = [(3, 8), (3, 24), (3, 72), (6, 8), (6, 24)]
SEEDS = [SEED, SEED + 1]
...
def main(workers=6, quick=False):
    ...
    arms = ["sgd_grow_noise"] if quick else list(ARMS)
    ...
    results, out_path = [], os.path.join(RESULTS_DIR, "scan_multidir.json")
