#!/usr/bin/env bash
# /gcp_run: latency-spike diagnostic (divergence vs slow-bin correlation) on a fresh VM.
set -uo pipefail
...
VM=exp-$(date +%Y%m%d-%H%M%S)-vjf-diag
...
gcloud compute instances create "$VM" --project=$PROJECT --zone=$ZONE $IMP \
  ...
  --labels=purpose=experiment,created-by=claude,mode=git,exp=diag >>"$LOG" 2>&1 || { echo "create failed" | tee -a "$LOG"; exit 1; }
...
gcloud compute scp --project=$PROJECT --zone=$ZONE $IMP \
  "$REPO/experiments/lc_poisson_stream/timing_diag.py" "$VM:~/vjf/timing_diag.py" >>"$LOG" 2>&1
echo "=== RUN diag ===" | tee -a "$LOG"
SSH 'cd ~/vjf && source .venv/bin/activate && python timing_diag.py' 2>&1 | tee -a "$LOG"
